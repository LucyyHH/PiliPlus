...
class DownloadForegroundService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        when (intent?.action) {
            ...
            ACTION_UPDATE -> updateNotification(
                title = intent.getStringExtra(EXTRA_TITLE) ?: "正在下载...",
                progress = intent.getLongExtra(EXTRA_PROGRESS, 0),
                total = intent.getLongExtra(EXTRA_TOTAL, 0)
            )
            ACTION_STOP -> stopDownloadForeground()
        }
    ...
    private fun updateNotification(title: String, progress: Long, total: Long) {
        ...
        manager.notify(NOTIFICATION_ID, createNotification(title, progress, total))
    }
